...
// src/handlers/send-alert.ts
var logger2 = createLogger("alert-service");
var awsConfig = {
...
var dynamoClient = DynamoDBDocumentClient.from(new DynamoDBClient(awsConfig));
var ALERTS_TABLE = process.env.ALERTS_TABLE || "cloudops-alerts";
var ALERTS_TOPIC_ARN = process.env.ALERTS_TOPIC_ARN || "arn:aws:sns:us-east-1:000000000000:cloudops-alerts";
async function sendAlert(input) {
  ...
  logger2.info("Sending alert", { alertId: alert.id, type: alert.type, severity: alert.severity });
  await storeAlert(alert);
  await publishToSNS(alert);
  logger2.info("Alert sent successfully", { alertId: alert.id });
  return alert;
}
...
async function publishToSNS(alert) {
  ...
  try {
    ...
    logger2.debug("Published to SNS", { alertId: alert.id });
  } catch (error) {
    logger2.error("Failed to publish to SNS", { alertId: alert.id, error });
  }
}
async function getAlerts(accountId, limit = 50) {
...
async function acknowledgeAlert(alertId, accountId) {
  logger2.info("Acknowledging alert", { alertId, accountId });
}
async function handler(event) {
  if (event.Records) {
    for (const record of event.Records) {
      const alertInput = JSON.parse(record.body);
