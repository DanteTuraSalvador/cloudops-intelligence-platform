...
// src/handlers/collect-metrics.ts
var collect_metrics_exports = {};
__export(collect_metrics_exports, {
  collectMetrics: () => collectMetrics,
  handler: () => handler
});
module.exports = __toCommonJS(collect_metrics_exports);
...
var logFormat = import_winston.default.format.combine(
  import_winston.default.format.timestamp({ format: "YYYY-MM-DDTHH:mm:ss.SSSZ" }),
...
var logger = createLogger("cloudops");
...
var dateStringSchema = external_exports.string().datetime({ message: "Invalid ISO 8601 date string" });
var accountIdSchema = external_exports.string().min(1).max(100).regex(/^[a-zA-Z0-9-_]+$/, "Account ID must be alphanumeric with dashes and underscores");
var awsAccountIdSchema = external_exports.string().length(12).regex(/^\d{12}$/, "AWS Account ID must be exactly 12 digits");
...
var metricQuerySchema = external_exports.object({
  accountId: accountIdSchema,
  metricType: metricTypeSchema.optional(),
  startTime: dateStringSchema,
  endTime: dateStringSchema,
  limit: external_exports.coerce.number().int().positive().max(1e3).optional()
});
var metricSchema = external_exports.object({
  ...
  dimensions: external_exports.record(external_exports.string()).optional(),
  tags: external_exports.record(external_exports.string()).optional()
});
var costQuerySchema = external_exports.object({
  accountId: accountIdSchema,
  startDate: external_exports.string().regex(/^\d{4}-\d{2}-\d{2}$/, "Date must be in YYYY-MM-DD format"),
  endDate: external_exports.string().regex(/^\d{4}-\d{2}-\d{2}$/, "Date must be in YYYY-MM-DD format")
});
var alertSeveritySchema = external_exports.enum(["info", "warning", "error", "critical"]);
var alertTypeSchema = external_exports.enum(["anomaly", "budget", "threshold", "recommendation"]);
var alertRuleSchema = external_exports.object({
  name: external_exports.string().min(1).max(100),
  type: alertTypeSchema,
  condition: external_exports.object({
    metricType: metricTypeSchema.optional(),
    operator: external_exports.enum(["gt", "gte", "lt", "lte", "eq"]),
  ...
  actions: external_exports.array(
    external_exports.object({
      type: external_exports.enum(["email", "sns", "webhook"]),
...
function getTTL(daysFromNow = 90) {
...
// src/handlers/collect-metrics.ts
var logger2 = createLogger("data-collector:metrics");
var awsConfig = {
...
var cloudwatchClient = new import_client_cloudwatch.CloudWatchClient(awsConfig);
var dynamoClient = import_lib_dynamodb.DynamoDBDocumentClient.from(new import_client_dynamodb.DynamoDBClient(dynamoConfig));
var METRICS_TABLE = process.env.METRICS_TABLE || "cloudops-metrics";
var METRICS_TO_COLLECT = [
...
async function collectMetrics(accountId = "demo-account") {
  const endTime = /* @__PURE__ */ new Date();
  const startTime = new Date(endTime.getTime() - 5 * 60 * 1e3);
  logger2.info("Starting metrics collection", {
    accountId,
  ...
  for (const metricDef of METRICS_TO_COLLECT) {
    try {
      ...
      if (process.env.USE_MOCK_DATA === "true") {
        ...
        logger2.debug("Using mock data", { metricType: metricDef.metricType });
      } else {
        metricValue = await fetchCloudWatchMetric(metricDef, startTime, endTime);
        logger2.debug("Fetched real CloudWatch data", { metricType: metricDef.metricType });
      }
      const metric = {
      ...
      logger2.debug("Stored metric", { metricType: metricDef.metricType, value: metricValue });
    } catch (error) {
      logger2.error("Failed to collect metric", {
        metricType: metricDef.metricType,
        error
      });
    }
  }
  logger2.info("Metrics collection completed", { accountId });
}
async function fetchCloudWatchMetric(metricDef, startTime, endTime) {
  try {
    const command = new import_client_cloudwatch.GetMetricDataCommand({
      MetricDataQueries: [
        {
          Id: "m1",
          MetricStat: {
    ...
    logger2.warn("No CloudWatch data available for metric", {
      namespace: metricDef.namespace,
      metricName: metricDef.metricName
    });
    return 0;
  } catch (error) {
    logger2.error("Failed to fetch CloudWatch metric", {
      namespace: metricDef.namespace,
      metricName: metricDef.metricName,
      error
    });
...
function generateMockMetricValue(metricType) {
  const mockValues = {
    CPUUtilization: () => 20 + Math.random() * 60,
    // 20-80%
    MemoryUtilization: () => 40 + Math.random() * 40,
    // 40-80%
    NetworkIn: () => Math.floor(1e6 + Math.random() * 5e6),
    // 1-6 MB
    NetworkOut: () => Math.floor(5e5 + Math.random() * 25e5),
    // 0.5-3 MB
    DiskReadOps: () => Math.floor(100 + Math.random() * 400),
    // 100-500 ops
    DiskWriteOps: () => Math.floor(50 + Math.random() * 200),
    // 50-250 ops
    DatabaseConnections: () => Math.floor(10 + Math.random() * 50),
    // 10-60 connections
    LambdaInvocations: () => Math.floor(1e3 + Math.random() * 9e3),
    // 1000-10000 invocations
    LambdaDuration: () => 50 + Math.random() * 450,
    // 50-500ms
...
async function handler(event) {
  logger2.info("Lambda triggered", { event });
